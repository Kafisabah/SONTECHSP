    ...
    **Doğrular: Gereksinimler 2.3**
    ...
    def test_veritabani_url_alani_butunlugu(self, gecici_dizin):
        ...
        for veritabani_url in test_urls:
            ...
            ayarlar = {"veritabani_url": veritabani_url, "ortam": "dev", "log_seviyesi": "INFO"}
    ...
    @given(
        st.dictionaries(
            st.text(min_size=1, max_size=20), st.one_of(st.text(), st.integers(), st.booleans()), min_size=1, max_size=5
        )
    )
