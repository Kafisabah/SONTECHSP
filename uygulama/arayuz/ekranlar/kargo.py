...
class Kargo(TemelEkran):
    ...
    def tasiyici_secimi_grubu_olustur(self):
        ...
        self.tasiyici_combo.addItems([
            "Yurtiçi Kargo", "MNG Kargo", "Aras Kargo", "PTT Kargo", 
            "UPS", "DHL", "FedEx", "Sürat Kargo"
        ])
    ...
    def kargo_tablosunu_guncelle(self):
        ...
        try:
            ...
            for row, kargo in enumerate(self.kargo_verileri):
                ...
                self.kargo_tablo.setItem(row, 5, QTableWidgetItem(
                    UIYardimcilari.tarih_formatla(kargo["tarih"])
                ))
                self.kargo_tablo.setItem(row, 6, QTableWidgetItem(kargo["tasiyici"]))
                self.kargo_tablo.setItem(row, 7, QTableWidgetItem(
                    UIYardimcilari.para_formatla(kargo["tutar"])
                ))
