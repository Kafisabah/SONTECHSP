...
# Changelog:
# - İlk oluşturma
# - Import düzenlemesi ve kod analizi
...
from sontechsp.uygulama.cekirdek.kayit import KayitSistemi, kayit_al, kayit_sistemi_al
...
class TestPyQt6UygulamaBaslatmaBütünlügü:
    ...
    **Validates: Requirements 1.2, 3.1, 3.2, 3.3**
    ...
    def test_log_sistemi_temel_işlevsellik(self):
        ...
        test_log_sistemi = KayitSistemi()

        # Logger al
        logger = kayit_al("test")
    ...
    def test_ana_pencere_import_edilebilir(self):
        ...
        try:
            from sontechsp.uygulama.ana import (
                AnaPencere,
                merkezi_hata_yakalayici,
                uygulama_baslat,
            )
    ...
    **Validates: Requirements 3.4**
    ...
    def test_ana_py_sadece_bootstrap_içerir(self):
        ...
        for kelime in yasakli_kelimeler:
            assert kelime.lower() not in ana_py_içerik.lower(), f"ana.py iş kuralı içermemeli: {kelime}"

        # Bootstrap anahtar kelimeler mevcut olmalı
        gerekli_kelimeler = ["QApplication", "AnaPencere", "uygulama_baslat", "kayit_sistemi", "merkezi_hata"]

        for kelime in gerekli_kelimeler:
            assert kelime in ana_py_içerik, f"Bootstrap kodu eksik: {kelime}"
    ...
    def test_ana_py_dosya_boyut_sınırı(self, dosya_satir_sayisi):
        ...
        assert len(kod_satirlari) <= 120, f"ana.py {len(kod_satirlari)} satır, maksimum 120 olmalı"
...
class TestMerkeziHataYönetimi:
    ...
    **Validates: Requirements 3.5, 7.4**
    ...
    def test_alan_hatasi_yapısı(self, alan_adi, hata_mesaji, deger):
        ...
        # Temel özellikler
        assert isinstance(hata, SontechHatasi)
    ...
    def test_merkezi_hata_yakalayici_mevcut(self):
        ...
        with patch("sontechsp.uygulama.ana.kayit_al") as mock_logger_al:
