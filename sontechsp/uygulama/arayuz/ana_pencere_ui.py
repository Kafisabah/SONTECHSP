...
def logo_alani_ekle(layout) -> None:
    """
    Sol panele logo alanı ekler

    Args:
        layout: Sol panel layout'u
    """
    try:
        from sontechsp.uygulama.arayuz.kaynaklar import logo_yukle
        ...
        # Logo yükle
        logo_pixmap = logo_yukle("logo_orta.png", genislik=100)

        if logo_pixmap:
            logo_label.setPixmap(logo_pixmap)
        else:
            # Logo yüklenemezse metin göster
            logo_label.setText("SONTECHSP")
        ...
        # Şirket adı (küçük)
        sirket_label = QLabel("POS + ERP + CRM")
        sirket_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        sirket_label.setStyleSheet(
            """
            QLabel {
                font-size: 9px;
                color: #7f8c8d;
        ...
        logo_layout.addWidget(sirket_label)

        layout.addWidget(logo_container)

    except Exception as e:
        # Logo yüklenemezse basit metin alanı ekle
        basit_logo = QLabel("SONTECHSP")
        basit_logo.setAlignment(Qt.AlignmentFlag.AlignCenter)
        basit_logo.setStyleSheet(
            """
            QLabel {
                font-weight: bold;
                font-size: 14px;
        ...
        layout.addWidget(basit_logo)


def pencere_ikonu_ayarla(pencere) -> None:
    """
    Ana pencereye ikon ayarlar
    ...
    try:
        ...
        # Pencere ikonu yükle
        pencere_ikonu = ikon_yukle("logo_ikon_32.png", boyut=32)

        if pencere_ikonu:
            pencere.setWindowIcon(pencere_ikonu)
...
def modul_listesi_al() -> List[Tuple[str, str]]:
    ...
    return [
        ("stok", "Stok Yönetimi"),
        ("pos", "POS Satış (Yeni Tasarım)"),  # Yeni tasarım vurgusu
        ("crm", "CRM"),
...
def hakkinda_goster(pencere) -> None:
    ...
    """
    QMessageBox.about(
        pencere, "SONTECHSP Hakkında", "SONTECHSP v0.1.0\n\n" "POS + ERP + CRM Sistemi\n" "Python + PyQt6 + PostgreSQL"
    )
