...
class AlanHatasi(SontechHatasi):
    ...
    def __init__(self, alan_adi: str, mesaj: str, alan_degeri: Any = None, beklenen_tip: Optional[str] = None):

        self.alan_adi = alan_adi
        self.deger = alan_degeri
        self.beklenen_tip = beklenen_tip

        # Türkçe mesaj oluştur
        turkce_mesaj = f"'{alan_adi}' alanında hata: {mesaj}"

        # Ek bilgi hazırla
        ek_bilgi = {"alan": alan_adi, "deger": alan_degeri}
        if beklenen_tip:
            ek_bilgi["beklenen_tip"] = beklenen_tip

        super().__init__(mesaj=turkce_mesaj, hata_kodu="ALAN_HATASI", seviye=HataSeviyesi.DUSUK, ek_bilgi=ek_bilgi)
...
class IsKuraliHatasi(SontechHatasi):
    ...
    def __init__(self, mesaj: str, kural_adi: Optional[str] = None):
        ...
        super().__init__(mesaj=turkce_mesaj, hata_kodu=hata_kodu, seviye=HataSeviyesi.ORTA, ek_bilgi=ek_bilgi)
...
class VeritabaniHatasi(SontechHatasi):
    ...
    def __init__(self, mesaj: str, sql_hata: Optional[str] = None, tablo_adi: Optional[str] = None):
        ...
        super().__init__(mesaj=turkce_mesaj, hata_kodu="VERITABANI_HATA", seviye=HataSeviyesi.YUKSEK, ek_bilgi=ek_bilgi)
...
class DogrulamaHatasi(SontechHatasi):
    ...
    def __init__(self, kural: str, mesaj: str, nesne_id: Optional[str] = None):

        self.kural = kural
        self.nesne_id = nesne_id

        # Türkçe mesaj oluştur
        turkce_mesaj = f"'{kural}' kuralı ihlal edildi: {mesaj}"
        hata_kodu = "DOGRULAMA_HATASI"

        # Ek bilgi hazırla
        ek_bilgi = {"kural_adi": kural}
        if nesne_id:
            ek_bilgi["nesne_id"] = nesne_id
...
class MigrationHatasi(SontechHatasi):
    ...
    def __init__(self, mesaj: str, migration_version: Optional[str] = None):
        ...
        super().__init__(mesaj=turkce_mesaj, hata_kodu="MIGRATION_HATA", seviye=HataSeviyesi.YUKSEK, ek_bilgi=ek_bilgi)
...
class VeriYuklemeHatasi(SontechHatasi):
    ...
    def __init__(self, mesaj: str, veri_tipi: Optional[str] = None):
        ...
        super().__init__(mesaj=turkce_mesaj, hata_kodu="VERI_YUKLEME_HATA", seviye=HataSeviyesi.ORTA, ek_bilgi=ek_bilgi)
...
class EntegrasyonHatasi(SontechHatasi):
    ...
    def __init__(self, sistem: str, mesaj: str, detay: Optional[str] = None):

        self.sistem = sistem
        self.hata_detayi = detay

        # Türkçe mesaj oluştur
        turkce_mesaj = f"'{sistem}' sistemi ile entegrasyon hatası: {mesaj}"

        # Ek bilgi hazırla
        ek_bilgi = {"sistem": sistem, "detay": detay}

        super().__init__(
            mesaj=turkce_mesaj,
            hata_kodu="ENTEGRASYON_HATASI",
...
class POSHatasi(SontechHatasi):
    ...
    def __init__(self, mesaj: str, islem_tipi: Optional[str] = None, hata_detayi: Optional[str] = None):
        ...
        super().__init__(mesaj=turkce_mesaj, hata_kodu="POS_HATA", seviye=HataSeviyesi.ORTA, ek_bilgi=ek_bilgi)
...
class BarkodHatasi(POSHatasi):
    ...
    def __init__(self, mesaj: str, barkod: Optional[str] = None):
        ...
        super().__init__(mesaj=f"Barkod hatası: {mesaj}", islem_tipi="barkod_okuma", hata_detayi=str(ek_bilgi))
...
class StokHatasi(POSHatasi):
    ...
    def __init__(
        ...
        super().__init__(mesaj=f"Stok hatası: {mesaj}", islem_tipi="stok_kontrolu", hata_detayi=str(ek_bilgi))
...
class OdemeHatasi(POSHatasi):
    ...
    def __init__(self, mesaj: str, odeme_turu: Optional[str] = None, tutar: Optional[float] = None):
        ...
        super().__init__(mesaj=f"Ödeme hatası: {mesaj}", islem_tipi="odeme", hata_detayi=str(ek_bilgi))
...
class IadeHatasi(POSHatasi):
    ...
    def __init__(self, mesaj: str, satis_id: Optional[int] = None, iade_tutari: Optional[float] = None):
        ...
        super().__init__(mesaj=f"İade hatası: {mesaj}", islem_tipi="iade", hata_detayi=str(ek_bilgi))
...
class NetworkHatasi(SontechHatasi):
    ...
    def __init__(self, mesaj: str, endpoint: Optional[str] = None, timeout: Optional[int] = None):
        ...
        super().__init__(mesaj=turkce_mesaj, hata_kodu="NETWORK_HATA", seviye=HataSeviyesi.YUKSEK, ek_bilgi=ek_bilgi)
...
class YazdirmaHatasi(POSHatasi):
    ...
    def __init__(self, mesaj: str, yazici_adi: Optional[str] = None, fis_no: Optional[str] = None):
        ...
        super().__init__(mesaj=f"Yazdırma hatası: {mesaj}", islem_tipi="yazdirma", hata_detayi=str(ek_bilgi))
...
class HataYoneticisi:
    ...
    def hata_isle(self, hata: Exception, ek_bilgi: Optional[Dict[str, Any]] = None) -> None:
        ...
        hata_bilgisi = {"hata_tipi": hata_tipi, "mesaj": str(hata), "zaman": self._simdi(), "ek_bilgi": ek_bilgi or {}}
    ...
    def hata_istatistikleri(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "en_sik_hata_tipi": (
                max(self.hata_sayaclari.items(), key=lambda x: x[1])[0] if self.hata_sayaclari else None
            ),
        }
    ...
    def kritik_hata_kontrol(self) -> bool:
        ...
        """
        kritik_hata_sayisi = sum(1 for hata in self.son_hatalar if "kritik" in hata.get("mesaj", "").lower())
