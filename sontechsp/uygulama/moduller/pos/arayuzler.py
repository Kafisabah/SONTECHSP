...
class ISatisRepository(ABC):
    ...
    def satis_odeme_ekle(self, satis_id: int, odeme_turu: OdemeTuru, 
                        tutar: Decimal, referans_no: Optional[str] = None) -> int:
        """Satışa ödeme ekler"""
        pass
...
class IIadeRepository(ABC):
    ...
    @abstractmethod
    def iade_olustur(self, orijinal_satis_id: int, terminal_id: int, 
                    kasiyer_id: int, neden: str) -> int:
        """Yeni iade kaydı oluşturur"""
        pass
    
    @abstractmethod
    def iade_satiri_ekle(self, iade_id: int, urun_id: int, adet: int, 
                        birim_fiyat: Decimal) -> int:
        """İade satırı ekler"""
        pass
...
class IOfflineKuyrukRepository(ABC):
    ...
    def kuyruga_ekle(self, islem_turu: IslemTuru, veri: Dict[str, Any], 
                    terminal_id: Optional[int] = None, kasiyer_id: Optional[int] = None, 
...
class IOdemeService(ABC):
    ...
    @abstractmethod
    def tek_odeme_yap(self, sepet_id: int, odeme_turu: OdemeTuru, 
                     tutar: Decimal) -> bool:
        """Tek ödeme işlemi yapar"""
        pass
    
    @abstractmethod
    def parcali_odeme_yap(self, sepet_id: int, 
                         odemeler: List[Dict[str, Any]]) -> bool:
...
class IOfflineKuyrukService(ABC):
    ...
    @abstractmethod
    def islem_kuyruga_ekle(self, islem_turu: IslemTuru, 
                          veri: Dict[str, Any]) -> bool:
